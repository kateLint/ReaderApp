...
import com.compose.readerapp.model.MUser
...
class LoginScreenViewModel: ViewModel() {
    val loadingState = MutableStateFlow(LoginState.IDLE)
            ...
            auth.signInWithEmailAndPassword(email, password)
                .addOnCompleteListener { task ->
                    if(task.isSuccessful){
                        Log.d("TAG", "signInWithEmailAndPassword: Yaaayya!${task.result.toString()}")
                        home()
                      //  TODO("take them home")
                    }else{
